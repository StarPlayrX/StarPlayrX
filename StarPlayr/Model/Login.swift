...
func session() -> String {
    let cookiesTask = cookies(channelid: "siriushits1", userid: userid!)
...
func getChannelList() -> (success: Bool, message: String, data: NSDictionary) {
    let channelsTask = channels(channeltype: "number", userid: userid! )
...
public func login(user: String, pass: String) -> (success: Bool, message: String, data: String) {
    ...
    let endpoint = insecure + local + ":" + String(port) + "/api/v2/login"
    let method = "login"

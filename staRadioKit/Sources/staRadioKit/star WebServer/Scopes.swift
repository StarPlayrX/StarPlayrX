...
public func scopes(_ scope: @escaping Closure) -> ((HttpRequest) -> HttpResponse) {
    return { _ in
        scopesBuffer[ProcessX.tid] = ""
        scope()
        return .raw(200, "OK", ["Content-Type": "text/html"], {
            try? $0.write([UInt8](("<!DOCTYPE html>"  + (scopesBuffer[ProcessX.tid] ?? "")).utf8))
        })
    }
}
...
private func evaluate(_ node: String, _ attrs: [String: String?] = [:], _ closure: Closure) {
    ...
    scopesBuffer[ProcessX.tid] = (scopesBuffer[ProcessX.tid] ?? "") + "<" + node
    
    // Save the current output before the nested scope evalutation.
    
    var output = scopesBuffer[ProcessX.tid] ?? ""
    ...
    if let inner = inner {
        scopesBuffer[ProcessX.tid] = output + ">" + (inner) + "</" + node + ">"
    } else {
        let current = scopesBuffer[ProcessX.tid]  ?? ""
        scopesBuffer[ProcessX.tid] = output + ">" + current + "</" + node + ">"
    }
